...
;;;;;;;;;;;;;;;; method environments ;;;;;;;;;;;;;;;;
  ...
  ;;; modified for problem 4 of final exam.
  ;;; find-method : Sym * Sym * Int -> Method 
  ;;; This function is used to find method with variable count. 
  ;;; This is used for variable overloading with different number
  ;;; of parameters.
  (define find-method
    (lambda (c-name name var-len)
      (let ((m-env (class->method-env (lookup-class c-name))))
        (let ((maybe-pair (assq name m-env)))
          (if (pair? maybe-pair) 
              (find-method-again name var-len m-env)
              (report-method-not-found name))))))
  
  ;;; The original find-method only provide method look up by name, 
  ;;; here in this function, we also considered the variable numbers 
  ;;; of the methods so that overloading of method is permitted.
  ;;; search-in-menv : Sym * Int * MethodEnv -> Method
  (define find-method-again 
    (lambda (method-name size m-env)
      (if (null? m-env) (report-method-not-found method-name)
          (let ((m-name (caar m-env)) (meth (cadar m-env)))
            (if (eqv? m-name method-name)
                (cases method meth
                  (a-method (vars body s-name f-names)
                            (if (eqv? (length vars) size)
                                meth
                                (find-method-again method-name size (cdr m-env)))))
                (find-method-again method-name size (cdr m-env)))))))
  
  ;;; This function is used to report method not found error. 
  ;;; It is added for method overloading method search. 
  (define report-method-not-found
    (lambda (method-name)
      (eopl:error 'find-method "method ~s not found." method-name)))
