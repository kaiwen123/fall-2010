...
starting at vertex 1. \fbox{1,2,4,6,5,7,8,3}
...
\item (10 points). Given the following directed, weighted graph, show
  ...
  to all other vertices, \fbox{See result at Table \ref{tbl:dij_alg}}.
...
\answer Please see result at Figure \ref{fig:heap-sort}.
...
a list of size 1. This means the call tree is $O(log_2(n))$. But no
two calls at the same level of the call tree process the same part of
the original list; thus, each level of calls needs only  time all
together The result is that the algorithm uses only $O(nlog_2(n))$
time. \\
An example of this case is: \fbox{3,1,0,2,5,4,7,6}

In the worst case, the two sublists have size 1 and n-1 (already
...
\answer All the inversions are : \fbox{$(1,4),(2,3),(2,4),(3,4)$}. \\
